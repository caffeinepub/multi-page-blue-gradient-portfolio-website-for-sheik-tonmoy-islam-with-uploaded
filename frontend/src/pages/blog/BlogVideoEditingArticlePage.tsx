...
export default function BlogVideoEditingArticlePage() {
  ...
  const article = blogArticles.find((a) => a.id === 'video-editing')!;

  useEffect(() => {
    setSEO({
      title: article.seoTitle,
      description: article.seoDescription,
      keywords: `video editing, ${article.category}, video production, post-production, video content`,
      ogTitle: article.seoTitle,
      ogDescription: article.seoDescription,
      ogImage: article.image,
      canonicalUrl: getCanonicalSiteUrl() + '/blog/video-editing',
    });
  }, [article]);
